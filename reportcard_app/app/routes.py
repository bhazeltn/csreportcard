...
def upload_files():
    if request.method == 'POST':
        ...
        # ... (rest of the form data retrieval) ...
        ...
        session_path = os.path.join(app.config['UPLOAD_FOLDER'], secure_filename(f"{report_date}_{session_name}"))
        ...
        if validation_results['success']:
            ...
            if existing_session:
                ...
                validation_results['session_exists'] = False
...
def confirm_session():
    ...
    if request.method == 'POST':
        ...
        if success:
            flash('Session data has been successfully saved to the database.', 'success')
            return redirect(url_for('upload_files')) 
        else:
