    ...
    for filename in os.listdir(directory_path):
        if filename.endswith('.xlsx'):  # Process only Excel files
            ...
            if report_type == 'Achievements Report':
                ...
                evaluations_df = process_evaluations(file_path, "/home/bradley/development/csreportcard/skill_names.csv")
                
                print("Evaluations Processed")
            else:
                print(f"Unknown report type for {filename}")
    
    ribbon_requirement_df = load_ribbon_requirements("/home/bradley/development/csreportcard/ribbons.csv")
